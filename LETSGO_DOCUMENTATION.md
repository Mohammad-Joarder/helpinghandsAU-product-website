...
| **Auth group** `(auth)` | `index` (welcome / entry), `sign-in`, `sign-up`, `forgot-password`, `verify-otp`, `role-select`, `admin-only`, `admin-compliance`, `driver-review-pending` (legacy **Redirect** to `/(driver)/onboarding-status`) |
...
- **Screens:** `/(auth)/` (welcome), `sign-in`, `sign-up`, `forgot-password`, `verify-otp`, `role-select`; admins: `admin-only`, `admin-compliance`; legacy `driver-review-pending` redirects into driver onboarding status.
...
**Wizard:** `app/(driver)/onboarding/*` — numbered steps `step1-personal` … `step9-submitted` (`lib/driverOnboardingSteps.ts`) plus **`verification-hub`** for compliance sub-flows (phone OTP, ABN, device, etc., per flags).

**Server-side submission gate:** `submit-driver-onboarding` enforces document presence, bank formatting, minimum licence/insurance horizons, and **feature-flagged** requirements (phone OTP, email confirmation, ABN, OCR, selfie face match, vehicle inspection, fraud scan, device registration, audit).

**Compliance edges (representative):**

- `driver-phone-send-otp` / `driver-phone-verify-otp`
- `abr-lookup-abn`
- `extract-license-ocr`
- `compare-driver-face-images`
- `register-driver-device`
- `evaluate-driver-fraud`
- `compliance-expiry-scan` (cron)
- `admin-compliance-drivers` / `admin-compliance-action`

**In-app admin:** `/(auth)/admin-compliance` for users with `profiles.role = 'admin'`.
...
- **Routes:** `admin-only` (explains primary admin work is **web admin panel**; links into mobile **driver compliance queue**), `admin-compliance` (in-app queue for `profiles.role = 'admin'`).
- **Edges:** `admin-compliance-drivers`, `admin-compliance-action`, `admin-refund-trip`
...
| `driver-phone-send-otp` / `driver-phone-verify-otp` | Phone verification |
| `abr-lookup-abn` | Australian Business Register |
| `extract-license-ocr` | Licence OCR pipeline |
| `compare-driver-face-images` | Selfie vs licence heuristic |
...
| `admin-compliance-drivers` / `admin-compliance-action` | Admin queue + actions |
